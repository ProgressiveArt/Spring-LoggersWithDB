...
public class App {
    ...
    private void logEvents (ApplicationContext ctx) throws IOException {
        ...
        event = ctx.getBean(Event.class);
        logEvent(EventType.SOMEEVENTTYPEONE, event, "SOMEEVENTTYPEONE event for user 1");

        event = ctx.getBean(Event.class);
        logEvent(EventType.SOMEEVENTTYPETWO, event, "SOMEEVENTTYPETWO event for user 1");
    }
